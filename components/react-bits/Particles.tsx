...
const Particles: React.FC<ParticlesProps> = ({
  ...
  useEffect(() => {
    ...
    const effectiveDist = Math.max(cameraDistance, particleSpread * 4);
    camera.position.set(0, 0, effectiveDist);
    ...
    const resize = () => {
      ...
      gl.canvas.style.width = '100%';
      gl.canvas.style.height = '100%';
      camera.perspective({ aspect: (w || 1) / (h || 1) });
    };
    const ro = new ResizeObserver(resize);
    ro.observe(container);
    const initResizeId = requestAnimationFrame(resize);
    ...
    const update = (t: number) => {
      ...
      if (cw !== sizeRef.current.w || ch !== sizeRef.current.h) {
        ...
        renderer.setSize(cw || 1, ch || 1);
        gl.canvas.style.width = '100%';
        gl.canvas.style.height = '100%';
        camera.perspective({ aspect: (cw || 1) / (ch || 1) });
      }
